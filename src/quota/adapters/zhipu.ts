...
import { fetchJson, hostOf, numberOf } from '../http.ts'
import { QuotaQueryError } from '../types.ts'
import type { QuotaAdapter, QuotaMatchInput, QuotaQueryContext, QuotaQueryResult } from '../types.ts'
...
function windowMinutesOf(unit: number | undefined, count: number | undefined): number | undefined {
  if (unit === undefined || count === undefined || count <= 0) return undefined
  if (unit === 5) return count
  if (unit === UNIT_FIVE_HOUR) return count * 60
  if (unit === 1) return count * 24 * 60
  if (unit === UNIT_WEEKLY) return count * 7 * 24 * 60
  return undefined
}
 ...
 * takes those). */
function tierOf(unit: number | undefined, minutes: number | undefined): 'five_hour' | 'weekly' | 'monthly' | undefined {
  if (minutes !== undefined) {
    if (minutes <= 360) return 'five_hour'
    // Unit 6 names the weekly bucket however its number reads — 6/7 was
    // observed serving the weekly window, not seven weeks of it.
    if (unit === UNIT_WEEKLY) return 'weekly'
    return minutes <= WEEKLY_MAX_MINUTES ? 'weekly' : 'monthly'
  }
  if (unit === UNIT_FIVE_HOUR) return 'five_hour'
  if (unit === UNIT_WEEKLY) return 'weekly'
  return undefined
}
...
function usedPercentOf(limit: Record<string, unknown>): number | undefined {
...
function isInternational(input: QuotaMatchInput | QuotaQueryContext): boolean {
  if (input.provider === ROUTE_ZAI) return true
...
function quotaBase(input: QuotaMatchInput | QuotaQueryContext): string {
...
export const zhipuAdapter: QuotaAdapter = {
  ...
  async query(ctx): Promise<QuotaQueryResult> {
    ...
    const unclassified: QuotaWindow[] = []
    const observedTypes: string[] = []
    for (const raw of record.limits) {
      if (typeof raw !== 'object' || raw === null) continue
      const limit = raw as Record<string, unknown>
      if (typeof limit.type === 'string') observedTypes.push(limit.type)
      ...
      if (limit.type !== undefined && !KNOWN_LIMIT_TYPES.has(String(limit.type).toUpperCase())) continue
      ...
      if (tier === 'five_hour' && fiveHour === undefined) fiveHour = window
      else if (tier === 'weekly' && weekly === undefined) weekly = { ...window, tier: 'weekly' }
      else if (tier === 'monthly' && monthly === undefined) monthly = { ...window, tier: 'monthly' }
      else if (tier === undefined) unclassified.push(window)
    }
    ...
    for (const window of unclassified) {
      if (fiveHour === undefined) fiveHour = window
      else if (weekly === undefined) weekly = { ...window, tier: 'weekly' }
    }
