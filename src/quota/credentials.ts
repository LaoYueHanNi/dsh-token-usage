...
export type CredentialRecordReader = (key: string) => Promise<{ key?: string } | undefined>

/** The built-in directory fallback for the two known settings shapes. */
const BUILTIN_DIRECTORY: Record<string, ProviderDirectoryEntry> = {
  'deepseek-official': { settingsNs: 'llm-deepseek', settingsPath: [] },
}

/** pi-ai route → default credential record key (`llm-pi-ai/<route>`). */
function recordKeyOf(provider: string): string {
...
const KNOWN_ENV_NAMES: Record<string, string> = {
...
function defaultEnvNameOf(provider: string): string {
...
const CATALOG_BASE_URLS: Record<string, string> = {
...
function walkPath(section: unknown, path: readonly string[]): unknown {
 ...
 */
export function withCatalogBaseUrl(
  provider: string,
  resolved: ResolvedQuotaCredentials,
): ResolvedQuotaCredentials {
  if (resolved.baseUrl !== undefined) return resolved
