...
- **Web 设置统计页**：设置面板（侧栏底部齿轮）新增「Token 用量」页，展示总计卡片、按日/按模型汇总表和最近请求列表，带手动刷新。
...
设置面板（侧栏底部齿轮）里有一个「Token 用量」页，数据来自本插件注册的 HTTP 路由 `GET /token-usage/stats`（不占用 `/api` RPC 方法表——那是宿主封闭的传输层，插件数据通道走自己的 webServer 路由）。返回的 JSON 包含总计、按日/按模型聚合和最近 20 条请求：
