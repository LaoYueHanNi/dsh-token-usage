 ...
 */
import { fetchJson, hostOf, numberOf, toEpochMs } from "../http.js";
...
function quotaHost(ctx) {
    const host = hostOf(ctx.baseUrl);
    if (host === INTERNATIONAL_HOST || host?.endsWith(`.${INTERNATIONAL_HOST}`))
        return INTERNATIONAL_HOST;
    if (host === DOMESTIC_HOST || host?.endsWith(`.${DOMESTIC_HOST}`))
        return DOMESTIC_HOST;
...
function isPlaceholderLane(entry, percentField, statusField) {
    if (numberOf(entry[statusField]) !== 3)
        return false;
