 ...
 */
function balanceRootOf(baseUrl) {
...
export const deepseekBalanceAdapter = {
    id: 'deepseek-balance',
    label: 'DeepSeek Balance',
    routes: [ROUTE_OFFICIAL],
    matches(input) {
        if (input.provider === ROUTE_OFFICIAL)
            return true;
    ...
    async query(ctx) {
        ...
        const pick = infos.find(entry => typeof entry === 'object' && entry !== null && entry.currency === 'CNY') ?? infos.find(entry => typeof entry === 'object' && entry !== null);
        if (pick === undefined) {
