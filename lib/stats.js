...
function dayModelRows(rows) {
    return [...rows].sort((left, right) => left.day < right.day ? -1 : left.day > right.day ? 1 : left.model.localeCompare(right.model));
}
/** Map key of one crossed (day, model) cell. */
function crossKey(day, model) {
    return `${day}\n${model}`;
}
/**
...
export function summarizeRecords(records) {
    ...
    const byDayModel = dayModelRows(crossed.values());
...
export function filterSummary(summary, from, to, model) {
    if (from === undefined && to === undefined && model === undefined)
        return summary;
    const rows = summary.byDayModel.filter(row => (from === undefined || row.day >= from)
    ...
    const recent = summary.recent.filter(record => (start === undefined || record.time >= start)
