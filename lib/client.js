window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const CARD_FIELDS = ["pricingRegion"];
		/**
		...
		*/
		...
		const zh = {
			...
			"card.region.overseas": "海外（GitHub）",
			"card.hint": "切换到「海外」后从 GitHub 镜像拉取定价表；保存后立即重新同步，失败沿用旧镜像。",
		...
		const en = {
			...
			"card.regionDefault": "Default (domestic Gitee)",
			"card.region.domestic": "Domestic (Gitee)",
			"card.region.overseas": "Overseas (GitHub)",
			"card.hint": "Switching to Overseas pulls the pricing table from the GitHub mirror; saving re-syncs immediately, falling back to the previous mirror on failure.",
