...
export function formatQuotaMoney(value, unit) {
 ...
 */
export function quotaUsedPercent(window) {
 ...
 * lacking a total. */
export function quotaSeverityOf(window) {
    ...
    if (remaining < REMAIN_RED)
        return 'exhausted';
    if (remaining <= REMAIN_YELLOW)
        return 'warn';
    return 'ok';
}
/**
 ...
 */
export function finestQuotaWindow(windows) {
    const rank = { five_hour: 0, weekly: 1, monthly: 2, balance: 3 };
    let pick;
    for (const window of windows) {
        if (pick === undefined || rank[window.tier] < rank[pick.tier])
            pick = window;
    }
    return pick;
}
 ...
 */
export function quotaIconFillShare(window) {
    if (window === undefined)
        return 0;
    const remaining = quotaRemainingPercent(window);
    if (remaining !== undefined)
        return remaining / 100;
    if (window.remainingValue !== undefined && window.remainingValue > 0)
        return 1;
    return 0;
}
/**
 ...
 * plain label).
 */
export function quotaTriggerFigure(window) {
    if (window === undefined)
        return undefined;
    const remaining = quotaRemainingPercent(window);
    if (remaining !== undefined)
        return formatQuotaPercent(remaining);
 ...
 */
export function formatResetCountdown(resetAt, now) {
...
export function formatQuotaClock(ms) {
...
//# sourceMappingURL=quota-format.js.map