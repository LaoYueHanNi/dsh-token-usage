...
function FilterBar({ filters, models, onChange, t }) {
    ...
    return (_jsxs("div", { className: styles['filters'], children: [_jsx(MenuSelect, { ariaLabel: t('filter.quickRange'), value: quickValue, options: [
                    ...QUICK_OPTIONS,
                ...
                } }), _jsx(DateRangePicker, { from: filters.from, to: filters.to, onChange: next => onChange({ ...filters, ...next }), t: t }), _jsx(MenuSelect, { ariaLabel: t('filter.model'), grow: true, value: filters.model, options: [{ value: '', label: t('filter.allModels') }, ...models.map(model => ({ value: model, label: model }))], onChange: model => onChange({ ...filters, model }) })] }));
}
/**
