...
export function TrendChart({ rows, from, to, t }) {
    ...
    return (_jsxs("svg", { role: "img", "aria-label": t('chart.aria'), viewBox: `0 0 ${WIDTH} ${HEIGHT}`, className: styles.chart, onMouseLeave: () => setActive(null), children: [ticks.map(tick => (_jsxs("g", { children: [_jsx("line", { x1: LEFT, y1: y(tick), x2: WIDTH - RIGHT, y2: y(tick), className: styles.grid }), _jsx("text", { x: LEFT - 6, y: y(tick) + 3, textAnchor: "end", className: styles.tick, children: formatTokens(tick) })] }, tick))), _jsx("line", { x1: LEFT, y1: y(0), x2: WIDTH - RIGHT, y2: y(0), className: styles.axis }), _jsx("path", { d: path, className: styles.line }), points.map((point, index) => (_jsx("circle", { cx: x(index), cy: y(point.tokens), r: active === index ? radius + 2.5 : radius, className: active === index ? styles.dotActive : styles.dot }, point.day))), activePoint !== null
                ? (
                ...
                : null, points.map((point, index) => (_jsx("rect", { x: x(index) - hitWidth / 2, y: TOP, width: hitWidth, height: innerHeight, fill: "transparent", "aria-label": t('chart.pointLabel', { day: point.day, tokens: formatTokens(point.tokens) }), role: "button", tabIndex: 0, className: styles.hit, onMouseEnter: () => setActive(index), onFocus: () => setActive(index), onBlur: () => setActive(current => current === index ? null : current) }, point.day))), activePoint !== null
                ? (
                ...
                : null, labelIndices(points.length).map(index => (_jsx("text", { x: x(index), y: HEIGHT - 6, textAnchor: index === 0 ? 'start' : index === points.length - 1 ? 'end' : 'middle', className: styles.tick, children: points[index].day.slice(5) }, index)))] }));
}
