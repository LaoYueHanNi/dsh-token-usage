...
export function TrendChart({ rows, from, to }) {
    ...
    return (_jsxs("svg", { role: "img", "aria-label": "\u6BCF\u65E5\u603B token \u66F2\u7EBF", viewBox: `0 0 ${WIDTH} ${HEIGHT}`, className: styles.chart, children: [ticks.map(tick => (_jsxs("g", { children: [_jsx("line", { x1: LEFT, y1: y(tick), x2: WIDTH - RIGHT, y2: y(tick), className: styles.grid }), _jsx("text", { x: LEFT - 6, y: y(tick) + 3, textAnchor: "end", className: styles.tick, children: formatTokens(tick) })] }, tick))), _jsx("line", { x1: LEFT, y1: y(0), x2: WIDTH - RIGHT, y2: y(0), className: styles.axis }), _jsx("path", { d: path, className: styles.line }), points.map((point, index) => (_jsx("circle", { cx: x(index), cy: y(point.tokens), r: radius, className: styles.dot }, point.day))), labelIndices(points.length).map(index => (_jsx("text", { x: x(index), y: HEIGHT - 6, textAnchor: index === 0 ? 'start' : index === points.length - 1 ? 'end' : 'middle', className: styles.tick, children: points[index].day.slice(5) }, index)))] }));
}
