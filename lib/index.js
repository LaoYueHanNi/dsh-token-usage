...
import { recordFromCompaction, recordFromEvent } from "./usage-record.js";
import { autoSyncIfNeeded, syncHistory } from "./sync.js";
import { clearRecordCache, warmRecordCache } from "./record-cache.js";
import { ROLLUP_FILE_NAME, ROLLUP_TMP_FILE_NAME } from "./rollup.js";
import { createDirectoryGuardRoute, createFullSyncRoute, createMigrationRoute, createStatsRoute } from "./stats-route.js";
import { currencyOfRegion } from "./wire.js";
import { createQuotaRoute } from "./quota/quota-route.js";
import { QuotaService } from "./quota/quota-service.js";
import { ProviderTracker, resolveCurrentProvider } from "./quota/provider-tracking.js";
import { resolveQuotaCredentials, withCatalogBaseUrl, } from "./quota/credentials.js";
/**
...
export function apply(ctx, config = {}) {
    ...
    let fullSyncStatus = { status: 'idle' };
    ...
    const triggerFullSync = () => {
        ...
        void syncHistory({ persistence: ctx.sessionPersistence, log: target.log, recordCompaction }, (tick) => {
            ...
            if (result.added > 0)
                invalidateDerivedState(target.dir);
    ...
    let quotaDirectory;
    let quotaReadSettings = () => undefined;
    let quotaResolveCredential;
    let quotaReadRecord;
    ...
    ctx.inject(['credentials'], (credentialsCtx) => {
        credentialsCtx.effect(() => {
            quotaResolveCredential = async (ref) => (await credentialsCtx.credentials.resolve(credentialRef(ref)))?.value;
            // The record store is newer than the installed typings; read it
            // feature-detected (absent on older hosts — the env layer covers).
            const recordReader = credentialsCtx.credentials.readRecord;
     ...
     */
    const invalidateDerivedState = (dir) => {
    ...
    const start = () => {
        ...
        ctx.inject(['webServer'], (webCtx) => {
            ...
            webCtx.effect(() => webCtx.webServer.register(createStatsRoute(currentDir, { currency: () => currencyOfRegion(effectiveInput().pricingRegion), logger })), 'token-usage: stats route');
            ...
            webCtx.effect(() => webCtx.webServer.register(createFullSyncRoute(() => fullSyncStatus, triggerFullSync)), 'token-usage: full sync route');
            ...
            webCtx.effect(() => webCtx.webServer.register(createQuotaRoute((sessionId, providerHint) => quotaEnabled
                ? quotaService.snapshot(sessionId, providerHint)
                : Promise.resolve({ status: 'disabled', intervalSec: quotaIntervalSec }))), 'token-usage: quota route');
        });
    ...
    const startFromSource = () => { start(); requestSync(); };
