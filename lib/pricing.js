...
async function readJsonFile(dir, name, logger = consoleLogger) {
    let text;
    try {
...
async function readManualPricing(dir, logger = consoleLogger) {
 ...
 */
async function readSyncedPricing(dir, logger = consoleLogger) {
 ...
 */
export async function readPricingTable(dir, logger = consoleLogger) {
    const merged = { ...await readSyncedPricing(dir, logger) };
 ...
 */
export async function readUsdExchangeRate(dir, logger = consoleLogger) {
    const feed = coerceCloudPricing(await readJsonFile(dir, PRICING_CCSA_FILE, logger));
