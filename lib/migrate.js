 ...
 */
export async function copyData(oldDir, newDir, onProgress) {
 ...
 */
export async function cleanSource(oldDir, newDir, onProgress, logger = consoleLogger) {
    const names = await ownedFiles(oldDir);
    const removable = [];
    for (const name of names) {
    ...
    for (const name of removable) {
        await unlink(join(oldDir, name)).catch((error) => {
