...
定价表只认云端镜像；`pricing.json` 不再被读取，文件搬迁照旧（不主动删除用户磁盘上的数据）：

1. **读取简化**（`src/pricing.ts`）：`readPricingTable` 仅 `readSyncedPricing`；删除仅服务手工格式的 `PRICING_FILE`、`readManualPricing`、`coercePricingTable`、`isModelPricing`。顶部注释与 `wire.ts` 的 `PricingTable` JSDoc 同步去掉「两个文件合并」描述。`stats-route.ts`、`attachCosts`、client 展示层不动。
...
- 换来：`readPricingTable` 仅返回云端镜像内容；数据目录存在合法 `pricing.json` 时也不被读取，统计照常出数（手工条目对应的模型按云端价或未定价计，与原「整模型覆盖」路径相比，原本被手工覆盖的峰谷/档位若云端已带则自动恢复按规则计费）。`stats-route.ts`、`attachCosts`、client 展示层、定价表总览提案语义均不破。删除遗留文件的读取代码与相关测试后，全量测试通过；`node docs/decisions/check.mjs` 通过。
- 代价：
  ...
  - **已有用户升级后静默失效**：无法证明无人使用（npm 分发过），仅能从本机数据目录无此文件佐证影响极小。靠升级说明与提交信息把影响讲清；文件不删，用户随时可自行清理。
