...
- **DSH 极简柔白微光渐变（Monochrome Soft Glow）**：新增纯函数 `areaPath(lineD, xs, yZero): string`。SVG 内部注入三阶垂直线性渐变（顶部 25%、中间 8%、底部 0% 完全透明），闭合回基线形成微光面积填充。在深色模式下呈现清晰通透的磨砂微光，主线条辅以轻柔微投影，完全摒弃刺眼的高饱和彩光，沉稳贴合 DSH 官方极简设计系统。
- **静态点消隐与双层交互光环**：默认状态下不再对多点序列渲染静态实心球（仅在总点数等于 1 时渲染孤立锚点防空屏）；当鼠标悬浮或键盘聚焦某点时，动态呈现外层半透明柔白发光光晕（`dotHalo`，半径 6.5px，14% 透明度）与内层实体圆点（`dotActive`，半径 3px），彻底消除底线白球排成一排的粗糙感。
- **全图鼠标吸附与精致卡片 Tooltip**：SVG 感应层在移动时基于横坐标就近吸附最近数据点，准星竖线顺畅随动；每个点保留原有 `tabIndex={0}` 与 `aria-label`；Tooltip 卡片增强圆角（`rx="6"`）、外阴影与分层排版。
- **低扰网格线**：网格线调整为细密半透明微虚线（4 4 dash，50% opacity），y=0 基线为干净平直细实线。
