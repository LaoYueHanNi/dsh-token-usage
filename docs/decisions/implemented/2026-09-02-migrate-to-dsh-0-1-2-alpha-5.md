...
4. **persistence 面适配（首轮漏判的补修）**：`SyncPersistence` duck-type 对齐运行时句柄面——`list(options?)` 返回 `{ header: { id } }` 快照（此前是裸 `{ id }`，裸 `signal` 参数同时换成 `{ signal }` 选项对象），事件读取从 `inspect(id).events` 改为 `open(id, 'read')` → `handle.read(0, undefined)` → `handle.close()`（错误路径先 close 再抛，句柄生命周期仿宿主官方 cold-read 范本）；传参处经 `hostPersistence(ctx)` 做一次 `as unknown as` 结构断言跨过 npm 包的 tag 时刻类型——断言越过的是 tag 形态类型与 master 开发版运行时的代差而非形状错误，依据是 harness 工作区源码（宿主 bundle 的解析源）与实测一致。测试 mock 同步贴运行时真实面（此前 mock 自带 `inspect`，正是测试全绿掩盖运行时崩溃的原因）。
5. **回归**：typecheck / 496 tests / build / build:client 全量跑在 alpha.5 真实类型与依赖上，全绿。ALPHA4-TODO.md 底稿使命完成，随迁移提交删除。
...
- 代价：本分支实际兼容的是 alpha.5 tag 之后的 master 开发版宿主（session-persistence 句柄面）——在 npm/tag 形态的 alpha.5 宿主上 persistence 读取会崩（那边仍是 coordinator 面，无 `open` 有 `inspect`）；`^0.1.2-alpha.5` 的 peer 声明与 version 后缀都点名了并不匹配的宿主形态。因此决定**不发布 npm**：版本号 `0.3.13-dsh-0.1.2-alpha.5` 仅作为分支内部序号，插件通过 profile `link:` 直连本机开发宿主使用；待宿主发布包含该重构的下一个 prerelease（npm 包转为句柄面）后，改版本后缀点名那个宿主再发。
